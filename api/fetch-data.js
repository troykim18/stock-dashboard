...
async function fetchNaverQuote(code) {
  ...
  const realtimeUrl = `https://polling.finance.naver.com/api/realtime/domestic/stock/${cleanCode}`;
  try {
    ...
    if (r.ok) {
      ...
      if (price) {
        return {
          ...
          price,
          pct: pct || 0,
          change: toNumber(item?.compareToPreviousClosePrice || item?.cv),
          volume: toNumber(item?.accumulatedTradingVolume || item?.aq),
  ...
  try {
    const r = await fetch(`https://finance.naver.com/item/main.naver?code=${cleanCode}`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 stock-dashboard',
        'Accept': 'text/html'
      }
    ...
    const today = html.match(/<p class="no_today">[\s\S]*?<span class="blind">([^<]+)<\/span>/);
    const rate = html.match(/<span class="rate_info">[\s\S]*?<span class="blind">([+-]?[0-9.,]+)%<\/span>/);
    const price = toNumber(today?.[1]);
    const pct = toNumber(rate?.[1]);
    if (price) {
      return {
        symbol: cleanCode,
        price,
...
async function fetchPrices(symbols = [], krCodes = []) {
  ...
  await mapLimit(cleanKrCodes, 8, async code => {
    ...
    const yahoo = naver ? null : await fetchYahooChart(`${code}.KS`) || await fetchYahooChart(`${code}.KQ`);
    const quote = naver || yahoo;
    if (quote) krPrices[code] = quote;
  });

  return { prices, krPrices, timestamp: new Date().toISOString(), krSourcePriority: 'naver,yahoo' };
}
...
async function fetchQuote(symbol) {
  ...
  if (hit) return { c: hit.price, dp: hit.pct, source: hit.source, marketTime: hit.marketTime };
...
async function fetchEnglishNews(symbol) {
  ...
  const url = `https://finnhub.io/api/v1/company-news?symbol=${symbol}&from=${f(weekAgo)}&to=${f(today)}&token=${FINNHUB_API_KEY}`;
  const r = await fetch(url);
